...
def test_starter_category_seed(app_modules):
    ...
    try:
        categories = set(app_modules.metadata.get_categories(con))
        assert categories == STARTER_CATEGORIES
        assert "Housing" not in categories

        assert app_modules.metadata.get_subcategories(con, "Food") == [
            "Dining",
            "Coffee",
            "Delivery",
        ]
...
def test_income_and_ignored_transaction_type_behavior(app_modules):
